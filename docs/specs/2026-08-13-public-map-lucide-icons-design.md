...
>
> **일부 갱신 (2026-09-03)** — 아래 매핑 표의 `elevator`가 바뀌었다.
>
> | 시설 유형 code | 이 문서       | 현재                          |
> | -------------- | ------------- | ----------------------------- |
> | `elevator`     | `ArrowUpDown` | 커스텀 아이콘 (`mapIcons.ts`) |
>
> lucide에 엘리베이터가 없어 `ArrowUpDown`을 골랐는데 화살표 둘만으로는
> 정렬 아이콘으로 읽힌다. 나머지 시설 유형 넷은 이 문서대로다.
