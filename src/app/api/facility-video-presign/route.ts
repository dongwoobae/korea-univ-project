...
export async function POST(request) {
  ...
  try {
    ...
    if (!facilityId || !contentType) {
      return NextResponse.json({ error: "facilityId 또는 contentType 누락" }, { status: 400 });
    }
    if (!ALLOWED_TYPES.includes(contentType)) {
      return NextResponse.json({ error: "mp4, webm, mov 형식만 업로드 가능해요" }, { status: 400 });
    }
    if (fileSize > MAX_SIZE) {
      return NextResponse.json({ error: "파일 크기는 500MB 이하여야 해요" }, { status: 400 });
    }
    ...
    return NextResponse.json({ presignedUrl, publicUrl: `${PUBLIC_URL}/${key}` });
  } catch (err) {
